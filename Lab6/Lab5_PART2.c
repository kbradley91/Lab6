...
int main(void){
	...
	while(1){
		test = keypadScan();
		if(test != laggingValue&&test!=0xFF){
			// TODO: set new frequency
			laggingValue = test;
			*outputPORT = test;
		}
		else{
...
// 150MHz CPU Freq, 1 second Period (in uSeconds)
  ...
  // ConfigCpuTimer(&CpuTimer0, 150, 1000000);
   ConfigCpuTimer(&CpuTimer1, 150, 1000000*1/10000*.5); //1/10000 is the frequency we want to toggle timer1 to interrupt
...
interrupt void ADC_isr(void){
	...
	initADC();
	value = ADC_get();
	if(SRAMaddress <= 0x2FFFF){
			SRAMwrite(value);
			//DAC_set(value);
		}
	else{
		SRAMaddress = 0x200000;
		changeFunctions();
	}
	if(a == 0){
		//DAC_set(value);
